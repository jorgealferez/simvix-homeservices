...
const submitSchema = z.object({
  channel: z
    .enum(['REGISTRO_TELEMATICO', 'REGISTRO_PRESENCIAL', 'SEDE_ELECTRONICA', 'CORREO_POSTAL'])
    .optional(),
...
export async function POST(req: Request, { params }: Params) {
  try {
    ...
    if (!ctx.isPlatformAdmin && project.orgId && project.orgId !== ctx.activeOrgId) {
      return NextResponse.json({ error: 'No accesible desde esta organización' }, { status: 403 });
    }
    ...
    const submission = await prisma.submission.create({
      data: {
        ...
        fechaPresentacion: parsed.data.fechaPresentacion
          ? new Date(parsed.data.fechaPresentacion)
          : new Date(),
